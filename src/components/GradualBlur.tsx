...
const GradualBlur: React.FC<GradualBlurProps> = props => {
  ...
  const blurDivs = useMemo(() => {
    ...
    for (let i = 1; i <= config.divCount; i++) {
      ...
      // Aplicamos primero la curva elegida (linear, bezier, ease-in, etc.)
      progress = curveFunc(progress);

      // Asegurar que el progreso esté en [0,1]
      progress = Math.min(1, Math.max(0, progress));

      // Calculamos el blur de forma que al inicio sea muy sutil
      // y al final mucho más intenso.
      let blurValue: number;
      if (config.exponential) {
        // Curva más agresiva: casi sin blur al inicio, mucho al final
        const eased = progress * progress; // suaviza la subida inicial
        blurValue = Number(math.pow(2, eased * 3)) * 0.04 * currentStrength;
      } else {
        // Curva suave: poco blur al principio, aumentando de forma cuadrática
        const eased = progress * progress;
        blurValue = (0.02 + 0.12 * eased) * currentStrength;
      }
